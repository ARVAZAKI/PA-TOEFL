...
const SpeakingQuestion = forwardRef(function SpeakingQuestion({ onComplete, section, questions }: Props, ref) {
    ...
    const flatQuestions = questions.flatMap((speaking) => speaking.questions.map((q) => ({ ...q, speakingId: speaking.id })));

    const currentQuestion = flatQuestions[data.currentQuestionIndex];
    const currentReading = questions.find((r) => r.id === currentQuestion.speakingId)!;
    ...
    const handleSaveRecording = async (blob: Blob) => {
        const currentSpeakings = questions[data.currentQuestionIndex];
