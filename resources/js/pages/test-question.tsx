...
export default function TestQuestion() {
    ...
    const handleComplete = () => {
        ...
        setMessage(currentPage?.nextId === 'scoreboard' ? 'Section completed. Click Continue to view your scoreboard.' : 'Section completed. Click Continue to move to the next subtest.');
        setOpenDialog(true);
    };
    ...
    const handleButtonDialog = () => {
        if (pendingRoute) {
            router.visit(pendingRoute, { replace: true });
            setPendingRoute('');
        } else if (timeLeft <= 1) {
            handleDialogSubmit();
        }
